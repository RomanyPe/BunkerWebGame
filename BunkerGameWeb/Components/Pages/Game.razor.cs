...
#pragma warning restore CS8618 // Поле, не допускающее значения NULL, должно содержать значение, отличное от NULL, при выходе из конструктора. Рассмотрите возможность добавления модификатора "required" или объявления значения, допускающего значение NULL.
    ...
    private async Task HandleRestart()
    {
        try
        {
            // 1. Сначала сбрасываем данные на сервере (синглтон обычно всегда доступен)
            GameManager?.FullRestart();

            // 2. Очищаем сессию с проверкой на null
            if (SessionStorage != null)
            {
                await SessionStorage.DeleteAsync("PlayerId");
            }

            // 3. Переход на главную с полной перезагрузкой
            Navigation?.NavigateTo("/", forceLoad: true);
        }
        catch (Exception ex)
        {
            // Если ошибка всё же случилась, просто редиректим игрока через JS или логируем
